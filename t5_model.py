...
from model_args import global_args
from model_args import T5Args
from utils import T5Dataset, load_hf_dataset
from utils import compute_metric_ner, compute_metric_assert
try:
    import wandb
...
class T5Model:
    ...
    def eval_model(self, eval_data, output_dir=None, verbose=True, silent=False, prompt="", write_to_file="", **kwargs):
        """
        ...
        if self.args.evaluate_generated_text:
            ...
            if prompt == "prompt1":
                template_sentence = " is a negative drug effect"
            elif prompt in ["prompt2", "prompt3"]:
                template_sentence = "Yes, the patient suffered from "

            if prompt in ["prompt1", "prompt2", "prompt3"]:
                normalized_preds = []
                normalized_labels = []
                for pred, label in zip(preds, eval_data["target_text"].tolist()):
                    if template_sentence in pred:
                        normalized_preds.append(pred.replace(template_sentence, "").strip('.'))
                    else:
                        normalized_preds.append("none")
                    if template_sentence in label:
                        normalized_labels.append(label.replace(template_sentence, "").strip('.'))
                    else:
                        normalized_labels.append("none")
                result = self.compute_metrics(normalized_labels, normalized_preds, **kwargs)           
                self.results.update(result)
            ...
            if write_to_file != "":
                pred_df = eval_data.copy() 
                pred_df["predicted_text"] = preds      
                pred_df.to_csv(write_to_file, index=False)  
        if verbose:
